...
OUTPUT_DIR = os.path.join(NEXUS_DIR, "network_topology_analyzer_output")
...
class NetworkTopologyAnalyzer:
    ...
    def __init__(self):
        """Initialize the network_topology_analyzer"""
        try:
            os.makedirs(OUTPUT_DIR, exist_ok=True)
            logger.info("✨ NetworkTopologyAnalyzer initialized")
        except OSError as e:
            logger.error(f"Initialization failed: {e}")
            raise
    
    def analyze_network_patterns(self) -> Dict[str, Any]:
        """Analyze NETWORK patterns from MoIE history"""
        try:
            ...
            with open(history_path, 'r') as f:
                for line in f:
                    try:
                        ...
                        # Look for network keyword in content
    ...
    def generate_insights(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if analysis.get('total_occurrences', 0) > 0:
                insights.append(f"{analysis['total_occurrences']} instances of NETWORK detected")
                
                # Domain distribution
                domains = [p['domain'] for p in analysis.get('patterns', []) if 'domain' in p]
                unique_domains = len(set(domains))
                insights.append(f"NETWORK appears across {unique_domains} domains")
                
                # VDR analysis
                vdrs = [p['vdr'] for p in analysis.get('patterns', []) if 'vdr' in p and p['vdr'] > 0]
                if vdrs:
                    avg_vdr = sum(vdrs) / len(vdrs)
                    insights.append(f"Average VDR for NETWORK: {avg_vdr:.2f}")
